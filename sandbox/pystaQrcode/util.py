...
def _lost_point_level3(modules, modules_count):
  ...
  for row in xrange(modules_count):
    ...
    for col in modules_range_short:
      if (this_row[col] and not this_row[col + 1] and this_row[col + 2] and
          this_row[col + 3] and this_row[col + 4] and not this_row[col + 5] and
          this_row[col + 6]):
        lost_point += 40

  for col in xrange(modules_count):
    for row in modules_range_short:
      if (modules[row][col] and not modules[row + 1][col] and
          modules[row + 2][col] and modules[row + 3][col] and
          modules[row + 4][col] and not modules[row + 5][col] and
          modules[row + 6][col]):
        lost_point += 40
...
def optimal_data_chunks(data, minimum=4):
  ...
  データ内容に最適化されたQRDataチャンクを返すイテレータです。
...
class QRData:
  ...
  QR対応フォーマットでデータを保持します。現在、KANJIは取り扱っていません。
  """
  ...
  def __init__(self, data, mode=None, check_data=True):
    ...
    if mode is None:
      ...
      if check_data and mode < optimal_mode(data):  # pragma: no cover
        raise ValueError("Provided data can not be represented in mode "
                         "{0}".format(mode))
